...
function bound!(ctrl::Vector{Vector{Float64}}, ctrl_bound)
    for ck = 1:length(ctrl)
        for tk = 1:length(ctrl[1])
            ctrl[ck][tk] = (
                x ->
...
function initial_ctrl!(opt, ctrl0, dynamics, p_num, rng)
    ...
    for pj = 1:length(ctrl0)
        dynamics[pj].data.ctrl =
            [[ctrl0[pj][i, j] for j = 1:ctrl_length] for i = 1:ctrl_num]
    end
...
function initial_M!(measurement0, C_all, dim, p_num, M_num, rng)
    ...
    for pj = 1:length(measurement0)
        C_all[pj] = [[measurement0[pj][i, j] for j = 1:dim] for i = 1:M_num]
    end
...
function initial_LinearComb!(measurement0, B_all, basis_num, M_num, p_num, rng)
    ...
    for pj = 1:length(measurement0)
        B_all[pj] = [[measurement0[pj][i, j] for j = 1:basis_num] for i = 1:M_num]
    end
...
function initial_LinearComb!(measurement0, B_all, basis_num, M_num, p_num, rng)
    ...
    for pj = 1:length(measurement0)
        B_all[pj] = [[measurement0[pj][i, j] for j = 1:dim] for i = 1:M_num]
    end
