...
Kraus(K::AbstractVector, dK::AbstractMatrix, ρ0::AbstractMatrix) = Kraus{dm}(
    Kraus_dm(K, [[dK[i, j, :, :][1] for j = 1:size(dK, 2)] for i = 1:size(dK, 1)], ρ0),
    :noiseless,
    :free,
    :dm,
)
Kraus(K::AbstractVector, dK::AbstractMatrix, ψ0::AbstractVector) = Kraus{ket}(
    Kraus_pure(K, [[dK[i, j, :, :][1] for j = 1:size(dK, 2)] for i = 1:size(dK, 1)], ψ0),
    :noiseless,
    :free,
    :ket,
)
