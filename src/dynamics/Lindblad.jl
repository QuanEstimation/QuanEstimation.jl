...
Lindblad(data,N,C,R) = Lindblad{((N,C,R).|>eval)...}(data,N,C,R)
...
Lindblad(H0::AbstractMatrix, dH::AbstractVector, ρ0::AbstractMatrix, tspan::AbstractVector) = Lindblad(Lindblad_noiseless_free(H0, dH, ρ0, tspan), :noiseless, :free)
...
Lindblad(H0::AbstractVector, dH::AbstractVector, ρ0::AbstractMatrix, tspan::AbstractVector) = Lindblad(Lindblad_noiseless_timedepend(H0, dH, ρ0, tspan,), :noiseless, :timedepend)

Lindblad(H0::AbstractVector, dH::AbstractVector, ρ0::AbstractMatrix, tspan::AbstractVector, decay_opt::AbstractVector,γ::AbstractVector) = Lindblad(Lindblad_noisy_timedepend(H0, dH, ρ0, tspan, decay_opt, γ), :noisy, :timedepend)

Lindblad(H0::AbstractVecOrMat, dH::AbstractVector, Hc::AbstractVector, cc::AbstractVector, ρ0::AbstractMatrix, tspan::AbstractVector) = Lindblad(Lindblad_noiseless_controlled(H0, dH, ρ0, tspan, Hc, cc), :noiseless, :controlled)

Lindblad(H0::AbstractVecOrMat, dH::AbstractVector,Hc::AbstractVector, cc::AbstractVector, ρ0::AbstractMatrix, tspan::AbstractVector, decay_opt::AbstractVector,γ::AbstractVector) = Lindblad(Lindblad_noisy_controlled(H0, dH, ρ0, tspan, decay_opt, γ, Hc, cc), :noisy, :controlled)

Lindblad(H0::AbstractMatrix, dH::AbstractVector, ψ0::AbstractVector, tspan::AbstractVector) = Lindblad(Lindblad_noiseless_free(H0, dH, ψ0, tspan), :noiseless, :free, :ket)

Lindblad(H0::AbstractMatrix, dH::AbstractVector, ψ0::AbstractVector, tspan::AbstractVector, decay_opt::AbstractVector,γ::AbstractVector) = Lindblad(Lindblad_noisy_free(H0, dH, ψ0, tspan, decay_opt, γ), :noisy, :free, :ket)

Lindblad(H0::AbstractVector, dH::AbstractVector, ψ0::AbstractVector, tspan::AbstractVector) = Lindblad(Lindblad_noiseless_timedepend(H0, dH, ψ0, tspan,), :noiseless, :timedepend, :ket)

Lindblad(H0::AbstractVector, dH::AbstractVector, ψ0::AbstractVector, tspan::AbstractVector, decay_opt::AbstractVector,γ::AbstractVector) = Lindblad(Lindblad_noisy_timedepend(H0, dH, ψ0, tspan, decay_opt, γ), :noisy, :timedepend, :ket)

Lindblad(H0::AbstractVecOrMat, dH::AbstractVector, Hc::AbstractVector, cc::AbstractVector, ψ0::AbstractVector, tspan::AbstractVector) = Lindblad(Lindblad_noiseless_controlled(H0, dH, ψ0, tspan, Hc, cc), :noiseless, :controlled, :ket)

Lindblad(H0::AbstractVecOrMat, dH::AbstractVector, Hc::AbstractVector, cc::AbstractVector, ψ0::AbstractVector, tspan::AbstractVector, decay_opt::AbstractVector, γ::AbstractVector) = Lindblad(Lindblad_noisy_controlled(H0, dH, ψ0, tspan, decay_opt, γ, Hc, cc), :noisy, :controlled, :ket)
...
function evolve(H, decay_opt, γ, dt, tj)
    Ld = dt * liouvillian(H, decay_opt, γ, tj)
    exp(Ld)
end

function evolve(H, dt)
    freepart = liouville_commu(H)
...
function secondorder_derivative(H0, ∂H_∂x::Vector{Matrix{T}}, ∂2H_∂x::Vector{Matrix{T}}, ρ0::Matrix{T}, decay_opt::Vector{Matrix{T}}, γ, Hc::Vector{Matrix{T}}, ctrl::Vector{Vector{R}}, tspan) where {T<:Complex,R<:Real}

    para_num = length(∂H_∂x)
    ctrl_num = length(Hc)
    ctrl_interval = ((length(tspan)-1)/length(ctrl[1])) |> Int
    ctrl = [repeat(ctrl[i], 1, ctrl_interval) |>transpose |>vec for i in 1:ctrl_num]
...
function evolve(dynamics::Lindblad{noiseless, free, ket})
    ...
    U = exp(-im * H0 * Δt)
    psi_t = psi0
    ∂psi_∂x = [psi0 |> zero for i in 1:para_num]
    for t in 2:length(tspan)
        psi_t = U * psi_t
...
function evolve(dynamics::Lindblad{noisy, free, ket})
    ...
    ∂ρt_∂x = [ρt |> zero for i in 1:para_num]
    Δt = tspan[2] - tspan[1]
    expL = evolve(H0, decay_opt, γ, Δt, 1)
    ∂H_L = [liouville_commu(∂H_∂x[i]) for i in 1:para_num]
    for t in 2:length(tspan)
        ρt = expL * ρt
        ∂ρt_∂x = [-im * Δt * ∂H_L[i] * ρt for i in 1:para_num] + [expL] .* ∂ρt_∂x
...
function evolve(dynamics::Lindblad{noisy, timedepend, ket})
    ...
    for t in 2:length(tspan)
        Δt = tspan[t] - tspan[t-1] # tspan may not be equally spaced 
        expL = evolve(H0[t-1], decay_opt, γ, Δt, t)
        ρt =  expL * ρt
        ∂ρt_∂x = [-im * Δt * ∂H_L[i] * ρt for i in 1:para_num] + [expL] .* ∂ρt_∂x
    end
    ρt = exp(vec(H0[end])' * zero(ρt) ) * ρt
