...
function HCRB(ρ::Matrix{T}, ∂ρ_∂x::Vector{Matrix{T}}, C::Matrix{Float64}, eps=1e-6) where {T<:Complex}
    if length(∂ρ_∂x) == 1
...
function Holevo_bound(ρ::Matrix{T}, ∂ρ_∂x::Vector{Matrix{T}}, C::Matrix{Float64};eps=eps_default) where {T<:Complex}
