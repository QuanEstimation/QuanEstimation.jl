...
function QFIM_SLD(ρ::Matrix{T}, dρ::Vector{Matrix{T}};eps=eps_default) where {T<:Complex}
    ...
    ([0.5*ρ] .* (kron(LD_tp, reshape(LD_tp,1,p_num)) + kron(reshape(LD_tp,1,p_num), LD_tp))).|> tr .|>real 
end
...
function QFIM_pure(ρ::Matrix{T}, ∂ρ_∂x::Vector{Matrix{T}}) where {T<:Complex}
    ...
    ([0.5*ρ] .* (kron(SLD, reshape(SLD,1,p_num)) + kron(reshape(SLD,1,p_num), SLD))) .|> tr .|>real
end

#======================================================#
#################### calculate CFIM ####################
function CFIM(ρ, dρ;eps=eps_default)
    M = SIC(size(ρ)[1])
    m_num = length(M)
    p_num = length(dρ)
    [real(tr(ρ*M[i])) < eps ? zeros(ComplexF64, p_num, p_num) : (kron(tr.(dρ.*[M[i]]),reshape(tr.(dρ.*[M[i]]), 1, p_num))/tr(ρ*M[i])) for i in 1:m_num] |> sum .|>real
end

function CFIM(ρ, dρ, M;eps=eps_default)
    m_num = length(M)
    p_num = length(dρ)
    [real(tr(ρ*M[i])) < eps ? zeros(ComplexF64, p_num, p_num) : (kron(tr.(dρ.*[M[i]]),reshape(tr.(dρ.*[M[i]]), 1, p_num))/tr(ρ*M[i])) for i in 1:m_num] |> sum .|>real
end
...
## QFI
function QFIM(ρ::Matrix{T}, dρ::Matrix{T}; dtype=:SLD, eps=eps_default) where {T<:Complex}
    Symbol("QFIM_"*string(dtype))(ρ, dρ;eps=eps)
end

## QFIM 
function QFIM(ρ::Matrix{T}, dρ::Vector{Matrix}{T}; dtype=:SLD, eps=eps_default) where {T<:Complex}
    Symbol("QFIM_"*string(dtype))(ρ, dρ;eps=eps)
end
...
## QFI with exportLD
function QFIM(::Type{Val{:exportLD}}, ρ::Matrix{T}, dρ::Matrix{T}; dtype=:SLD, eps=eps_default) where {T<:Complex}
...
## QFIM with exportLD
function QFIM(::Type{Val{:exportLD}}, ρ::Matrix{T}, dρ::Vector{Matrix{T}}; dtype=:SLD, eps=eps_default) where {T<:Complex}
    F = QFIM(ρ,dρ;dtype=dtype, eps=eps)
    LD = eval(dtype)(ρ,dρ;eps=eps)
    return F, LD
end
