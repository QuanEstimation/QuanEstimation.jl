function update!(opt::StateOpt, alg::NM, obj, dynamics, output)
    ...
    if length(ini_state) > state_num
        ini_state = [ini_state[i] for i = 1:state_num]
    end
    for pj = 1:length(ini_state)
        nelder_mead[pj].data.ψ0 = [ini_state[pj][i] for i = 1:dim]
    end
    for pj = (length(ini_state)+1):state_num
    ...
    for ei = 1:(max_episode-1)
        ...
        for ni = 1:dim
            vec_ave[ni] = [nelder_mead[pk].psi[ni] for pk = 1:(state_num-1)] |> sum
        ...
        for nj = 1:dim
            vec_ref[nj] =
                vec_ave[nj] + ar * (vec_ave[nj] - nelder_mead[sort_ind[end]].psi[nj])
        end
        ...
        elseif fr < p_fit[sort_ind[end-1]]
            ...
            if fr <= p_fit[sort_ind[end]]
                ...
                for nl = 1:dim
                    vec_ic[nl] =
                        vec_ave[nl] -
                        ac * (vec_ave[nl] - nelder_mead[sort_ind[end]].psi[nl])
                end
                ...
                if fic > p_fit[sort_ind[end]]
                    ...
                    # shrink
                    vec_first = [nelder_mead[sort_ind[1]].psi[i] for i = 1:dim]
                    for pk = 1:state_num
                        for nq = 1:dim
                            nelder_mead[pk].psi[nq] =
                                vec_first[nq] +
                                as0 * (nelder_mead[pk].psi[nq] - vec_first[nq])
                        end
                        nelder_mead[pk].psi =
                            nelder_mead[pk].psi / norm(nelder_mead[pk].psi)
