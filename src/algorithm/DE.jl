function update!(opt::ControlOpt, alg::DE, obj, dynamics, output)
    ...
    for pj in 1:length(ini_population)
        populations[pj].data.ctrl = [[ini_population[pj][i,j] for j in 1:ctrl_length] for i in 1:ctrl_num]
    end
    if opt.ctrl_bound[1] == -Inf || opt.ctrl_bound[2] == Inf
        for pj in (length(ini_population)+1):p_num
            populations[pj].data.ctrl = [[2*rand(rng,)-1.0 for j in 1:ctrl_length] for i in 1:ctrl_num]
        end
    else
        a = opt.ctrl_bound[1]
        b = opt.ctrl_bound[2]
        for pj in (length(ini_population)+1):p_num
            populations[pj].data.ctrl = [[(b-a)*rand(rng,)+a for j in 1:ctrl_length] for i in 1:ctrl_num]
        end
    end
...
function update!(Sopt::StateOpt, alg::DE, obj, dynamics)
    ...
    for pj in 1:length(ini_population)
        populations[pj].data.ψ0 = [ini_population[pj][i] for i in 1:dim]
    end
    for pj in (length(ini_population)+1):p_num
        r_ini = 2*rand(rng,dim)-ones(dim)
        r = r_ini/norm(r_ini)
        phi = 2*pi*rand(rng,dim)
        populations[pj].data.ψ0 = [r[i]*exp(1.0im*phi[i]) for i in 1:dim]
    end
...
function update!(Mopt::MeasurementOpt{Projection}, alg::DE, obj, dynamics, output, C)
    ...
    M = [populations[1].C[i]*(populations[1].C[i])' for i in 1:M_num]
    ...
    for ei in 1:(max_episode-1)
        ...
        idx = findmax(p_fit)[2]
        append!(output.f_list, p_out[idx])
        M = [populations[idx].C[i]*(populations[idx].C[i])' for i in 1:M_num]
...
function update!(Mopt::MeasurementOpt{LinearComb}, alg::DE, obj, dynamics, output, POVM_basis, M_num)
...
function update!(Comopt::StateControlOpt, alg::DE, obj, dynamics)
    ...
    for pj in 1:p_num
        ...
        for ci in 1:ctrl_num
            for ti in 1:ctrl_length
                ctrl_mut[ci][ti] = populations[mut_num[1]].ctrl[ci][ti] +
                                   c * (populations[mut_num[2]].ctrl[ci][ti] -
