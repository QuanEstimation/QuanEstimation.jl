...
#### state optimization ####
function update!(opt::StateOpt, alg::AD_Adam, obj, dynamics, output)
    (; max_episode) = alg
    f_ini, f_comp = objective(obj, dynamics)
    set_f!(output, f_ini)
...
function update!(opt::Mopt_LinearComb, alg::AD_Adam, obj, dynamics, output)
    (; max_episode) = alg
...
function update!(opt::Mopt_LinearComb, alg::AD, obj, dynamics, output)
    (; max_episode) = alg
    (; POVM_basis, M_num) = opt
    basis_num = length(POVM_basis)
...
function update!(opt::Mopt_Rotation, alg::AD_Adam, obj, dynamics, output)
    (; max_episode) = alg
    (; POVM_basis) = opt
...
function update!(opt::Mopt_Rotation, alg::AD, obj, dynamics, output)
    (; max_episode) = alg
    (; POVM_basis) = opt
    dim = size(dynamics.data.ρ0)[1]
    M_num = length(POVM_basis)
