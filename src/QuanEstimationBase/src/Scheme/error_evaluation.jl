function error_evaluation(scheme::Scheme; verbose::Bool=true, input_error_scaling=1e-8, SLD_eps=1e-6, abstol=1e-6, reltol=1e-3)
    println("Error evaluation for $(typeof(scheme))")
    param_error = param_error_evaluation(scheme, input_error_scaling; abstol=abstol, reltol=reltol, verbose=verbose)
    eps_error = SLD_eps_error(scheme, SLD_eps)
    println("\nOverall error scaling ≈ ", param_error + eps_error)
end

function param_error_evaluation(scheme::Scheme{S, Lindblad{HT, DT, CT, Expm, P}, M, E}, input_error_scaling;verbose::Bool=true, abstol=1e-6, reltol=1e-3) where {S, HT, DT, CT, P, M, E}
    ...
    gs_c = grad[ctrl]
    gs_s = grad[rho]
    δF = sum([norm(g)*input_error_scaling for g in gs_c]) + sum([norm(g)*input_error_scaling for g in gs_s])
    if verbose
        println("\nError evaluation for Lindblad dynamics")
        println("Source: input data (ctrl, Hc, decay etc.) error scaling = $(input_error_scaling)")
        println("δF ≈ ", δF)
    end
...
function param_error_evaluation(scheme::Scheme{S, Lindblad{HT, DT, CT, Ode, P}, M, E}, input_error_scaling;verbose::Bool=true, abstol=1e-6, reltol=1e-3) where {S, HT, DT, CT, P, M, E}
...
function QFIM_with_error(scheme::Scheme;verbose::Bool=false, eps=GLOBAL_EPS)
    ...
    F = (
        [0.5 * rho] .*
        (kron(SLD_tp, reshape(SLD_tp, 1, p_num)) + kron(reshape(SLD_tp, 1, p_num), SLD_tp))
    ) .|> tr .|> real
    δF = real.(tr.(
        [0.5 * rho] .*
        (kron(SLD_tp_err, reshape(SLD_tp_err, 1, p_num)) + kron(reshape(SLD_tp_err, 1, p_num), SLD_tp_err))
    )) - F
    return F, δF
end 
    ...
    for fi = 1:dim
        for fj = 1:dim
            if val[fi] + val[fj] > eps
                ...
                SLD_eig_err[fi, fj] = 2 * (vec[:, fi]' * dρ * vec[:, fj]) / (val[fi] + val[fj])
            end
        end
    end
...
function SLD_with_error(ρ::Matrix{T}, dρ::Vector{Matrix{T}}; eps = GLOBAL_EPS) where {T<:Complex}
...
function QFIM_SLD_with_error(ρ::Matrix{T}, dρ::Matrix{T}; eps = GLOBAL_EPS) where {T<:Complex}
...
function QFIM_RLD_with_error(ρ::Matrix{T}, dρ::Matrix{T}; eps = GLOBAL_EPS) where {T<:Complex}
    RLD_tp = pinv(ρ, reltol = eps) * dρ
    F = tr(ρ * RLD_tp * RLD_tp')
    F |> real
end

function QFIM_LLD_with_error(ρ::Matrix{T}, dρ::Matrix{T}; eps = GLOBAL_EPS) where {T<:Complex}
...
#==========================================================#
####################### calculate QFIM #####################
function QFIM_SLD_with_error(ρ::Matrix{T}, dρ::Vector{Matrix{T}}; eps = GLOBAL_EPS) where {T<:Complex}
    p_num = length(dρ)
...
function QFIM_RLD_with_error(ρ::Matrix{T}, dρ::Vector{Matrix{T}}; eps = GLOBAL_EPS) where {T<:Complex}
    p_num = length(dρ)
...
function QFIM_LLD_with_error(ρ::Matrix{T}, dρ::Vector{Matrix{T}}; eps = GLOBAL_EPS) where {T<:Complex}
    p_num = length(dρ)
