...
AdaptiveStrategy(x::AbstractVector, p::AbstractArray; online=true) = AdaptiveStrategy(x, p, online)

function adapt_param!(scheme::Scheme{S,P,M,AdaptiveStrategy}, x) where {S,P,M}
    scheme.Parameterization.params = [x...]
end

function adapt_scheme!(scheme::Scheme{ST,PT,MT,AdaptiveStrategy}, x_list, M, W, eps) where {ST,PT,MT}
...
function adapt!(scheme::Scheme{ST,PT,MT,AdaptiveStrategy}; method = "FOP", savefile = false, max_episode::Int = 1000, W = nothing, eps=GLOBAL_EPS) where {ST,PT,MT}
    ...
    if method == "FOP"
        ...
        if savefile == false
            ...
            for ei in 1:max_episode
                p, x_out, res_exp, u = iter_FOP(p, p_num, para_num, x, x_list, u, rho_all, M, x_opt, ei)
    ...
    elseif method == "MI"
        if savefile == false
            y, xout = Int64[], Float64[]
            for ei in 1:max_episode
                p, x_out, res_exp, u = iter_MI(p, p_num, para_num, x, x_list, u, rho_all, M, ei)
                append!(xout, [x_out])
                append!(y, Int(res_exp+1))
            end
            savefile_false(p, xout, y)
