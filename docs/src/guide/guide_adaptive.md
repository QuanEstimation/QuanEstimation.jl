...
the adaptive estimation can be executed through
``` jl
...
generated. The package contains two methods for updating the tunable parameters. The first one is 
...
``` jl
H, dH = QuanEstimation.BayesInput(x, func, dfunc; channel="dynamics")
```
...
and 
``` jl
K, dK = QuanEstimation.BayesInput(x, func, dfunc; channel="Kraus")
```
...
and the code for calling offline strategy becomes `alg = QuanEstimation.DE(kwargs...)` 
...
**Example 9.2**  
In this example, the adaptive measurement scheme is designed for the MZI [[3,4]](#Hentschel2010). 
The input state is 
```math
\begin{align}
